...
static void CAN_Mode_Config(void)
{
    ...
    /*CAN单元初始化*/
    CAN_InitStructure.CAN_TTCM=DISABLE;			   //MCR-TTCM  关闭时间触发通讯模式使能
    ...
    /* CAN Baudrate = 1 MBps (1MB已为stm32的CAN最高速率) (CAN 时钟频率为 APB1 = 36 MHz) */
...
static void CAN_Filter_Config(void)
{
    ...
    /* 使能筛选器，按照标志的内容进行对比筛选，扩展ID不是如下的就抛弃掉，是的话，会存入FIFO0 */
...
//    CAN_FilterInitStructure.CAN_FilterIdLow= (((u32)0x1314<<3)|CAN_ID_EXT|CAN_RTR_DATA)&0xFFFF; //要筛选的ID地位
